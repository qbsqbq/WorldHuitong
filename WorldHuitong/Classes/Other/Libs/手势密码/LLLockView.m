...
//#define kCircleMargin 42.0              // 圆点离屏幕左边距
...
#define kLLBaseCircleNumber 10000       // tag基数（请勿修改）
#define kCircleMargin [UIScreen mainScreen].bounds.size.width / 9           // 圆点离屏幕左边距
#define kCircleDiameter 65.0            // 圆点直径
#define kLLCircleAlpha 1.0              // 圆点透明度
#define kLLLineWidth 3.0               // 线条宽
#define kLLLineColor [UIColor colorWithRed:77.0/255.0 green:76.0/255.0 blue:156.0/255.0 alpha:0.8] // 线条色蓝
//#define kLLLineColor [UIColor greenColor]
...
- (void)initCircles
{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        int x = kCircleMargin + (i%3) * (kCircleDiameter+(kScreenWidth-kCircleMargin*2- kCircleDiameter *3)/2);
        int y = kCircleMargin + (i/3) * (kCircleDiameter+(kScreenWidth-kCircleMargin*2- kCircleDiameter *3)/2);
//        LLLog(@"每个圆点位置 %d,%d", x, y);
        [button setFrame:CGRectMake(x, y, kCircleDiameter, kCircleDiameter)];
        
        [button setBackgroundColor:[UIColor clearColor]];
        [button setBackgroundImage:[UIImage imageNamed:@"circle_normal.png"] forState:UIControlStateNormal];
        [button setBackgroundImage:[UIImage imageNamed:@"circle_selected.png"] forState:UIControlStateSelected];
...
- (void)showErrorCircles:(NSString*)string
{
    ...
    for (UIButton* button in buttonArray) {
        if (button.selected) {
            [button setBackgroundImage:[UIImage imageNamed:@"circle_wrong.png"] forState:UIControlStateSelected];
        }
